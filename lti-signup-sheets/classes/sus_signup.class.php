	...
	class SUS_Signup extends Db_Linked {
		...
		public function cascadeDelete($usr_object) {
			...
			if ($o->begin_datetime >= util_currentDateTimeString_asMySQL()) {
				...
				if ($usr_object->username != $signup_user->username) {
					$body .= "\n(Cancelled by: " . $usr_object->first_name . " " . $usr_object->last_name . ")\n";
				}
		...
		public function renderAsHtmlSignupWithFullControls($userDisplayFullname, $userDisplayUsername) {
			...
			$rendered .= ' <span class="small" title="(username, when signed up)">(' . htmlentities($userDisplayUsername, ENT_QUOTES, 'UTF-8') . ', ' . util_datetimeFormatted($this->created_at) . ')</span> ';
