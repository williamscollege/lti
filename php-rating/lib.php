...
###
  function deleteAllRatings($db, $resource_id) {
...
###
  function deleteAllItems($db, $consumer_key, $resource_id) {

// Delete any ratings
    deleteAllRatings($db, $resource_id);

// Delete the items
    $prefix = DB_TABLENAME_PREFIX;
    $sql = <<< EOD
DELETE FROM {$prefix}item
WHERE (consumer_key = :consumer_key) AND (resource_id = :resource_id)
EOD;
...
###
  function getUserSummary($db, $consumer_key, $user_id) {

    $prefix = DB_TABLENAME_PREFIX;
    $sql = <<< EOD
SELECT c.lti_context_id, i.resource_id, i.max_rating, r.rating
FROM {$prefix}lti_context c INNER JOIN  {$prefix}item i ON c.consumer_key = i.consumer_key AND c.context_id = i.resource_id
  INNER JOIN {$prefix}rating r ON i.item_id = r.item_id
WHERE r.consumer_key = :consumer_key AND r.user_id = :user_id
EOD;
...
###
  function getUserRatings($db, $consumer_key, $context_id, $user_consumer_key, $user_id) {

    $prefix = DB_TABLENAME_PREFIX;
    $sql = <<< EOD
SELECT c.title, i.max_rating, r.rating
FROM {$prefix}lti_context c INNER JOIN  {$prefix}item i ON c.consumer_key = i.consumer_key AND c.context_id = i.resource_id
  INNER JOIN {$prefix}rating r ON i.item_id = r.item_id
WHERE c.consumer_key = :consumer_key AND  c.lti_context_id = :context_id AND r.consumer_key = :user_consumer_key AND r.user_id = :user_id
EOD;

    $query = $db->prepare($sql);
    $query->bindValue('consumer_key', $consumer_key, PDO::PARAM_STR);
    $query->bindValue('context_id', $context_id, PDO::PARAM_STR);
    $query->bindValue('user_consumer_key', $user_consumer_key, PDO::PARAM_STR);
    $query->bindValue('user_id', $user_id, PDO::PARAM_STR);
    $query->execute();
...
###
  function getContextRatings($db, $consumer_key, $context_id) {
...
FROM {$prefix}lti_context c INNER JOIN  {$prefix}item i ON c.consumer_key = i.consumer_key AND c.context_id = i.resource_id
  INNER JOIN {$prefix}rating r ON i.item_id = r.item_id
WHERE c.consumer_key = :consumer_key AND  c.lti_context_id = :context_id
EOD;
