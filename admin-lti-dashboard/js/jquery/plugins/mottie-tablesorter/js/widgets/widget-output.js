		...
		output = ts.output = {
			...
			process: function (c, wo) {
				...
				for (indx = 0; indx < c.columns; indx++) {
					...
					( !wo.output_hiddenColumns && c.$headerIndexed[indx].css('display') === 'none' && !c.$headerIndexed[indx].hasClass('tablesorter-scroller-hidden-column')  );
				}
				...
				if ($.isFunction(wo.output_callback) && !wo.output_callback(c, mydata)) {
					return;
				}
