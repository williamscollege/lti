...
!function (a) {
	...
	c.addWidget({
		...
		options: {
			...
			pager_removeRows: !1,
			pager_ajaxUrl: null,
			pager_customAjaxUrl: function (a, b) {
				return b
			},
			pager_ajaxError: null,
			pager_ajaxObject: {dataType: "json"},
			pager_processAjaxOnInit: !0,
			pager_ajaxProcessing: function (a) {
				return [0, [], null]
			},
		...
		init: function (d) {
		...
		}, bindEvents: function (d, e) {
			var f, g, h = e.pager, i = e.widgetOptions, j = e.namespace + "pager", k = i.pager_selectors;
			e.$table.off(j).on("filterInit filterStart ".split(" ").join(j + " "), function (b, c) {
				h.currentFilters = a.isArray(c) ? c : e.$table.data("lastSearch"), "filterStart" === b.type && i.pager_pageReset !== !1 && (e.lastCombinedFilter || "") !== (h.currentFilters || []).join("") && (h.page = i.pager_pageReset)
			}).on("filterEnd sortEnd ".split(" ").join(j + " "), function () {
				h.currentFilters = e.$table.data("lastSearch"), (h.initialized || h.initializing) && (e.delayInit && e.rowsCopy && 0 === e.rowsCopy.length && b.updateCache(d), b.updatePageDisplay(d, e, !1), c.applyWidget(d))
			}).on("disablePager" + j, function (a) {
				a.stopPropagation(), b.showAllRows(d, e)
			}).on("enablePager" + j, function (a) {
				a.stopPropagation(), b.enablePager(d, e, !0)
			}).on("destroyPager" + j, function (a, c) {
				a.stopPropagation(), b.destroyPager(d, e, c)
			}).on("updateComplete" + j, function (a, d, f) {
				if (a.stopPropagation(), d && !f && !h.ajax) {
					var g = e.$tbodies.eq(0).children("tr").not(e.selectorRemove);
					h.totalRows = g.length - (i.pager_countChildRows ? 0 : g.filter("." + e.cssChildRow).length), h.totalPages = Math.ceil(h.totalRows / h.size), g.length && e.rowsCopy && 0 === e.rowsCopy.length && b.updateCache(d), h.page >= h.totalPages && b.moveToLastPage(d, h), b.hideRows(d, e), b.changeHeight(d, e), b.updatePageDisplay(d, e, !1), c.applyWidget(d), b.updatePageDisplay(d, e)
				}
			}).on("pageSize refreshComplete ".split(" ").join(j + " "), function (a, c) {
				a.stopPropagation(), b.setPageSize(d, parseInt(c, 10) || h.setSize || 10, e), b.hideRows(d, e), b.updatePageDisplay(d, e, !1)
			}).on("pageSet pagerUpdate ".split(" ").join(j + " "), function (a, c) {
				a.stopPropagation(), "pagerUpdate" === a.type && (c = "undefined" == typeof c ? h.page + 1 : c, h.last.page = !0), h.page = (parseInt(c, 10) || 1) - 1, b.moveToPage(d, h, !0), b.updatePageDisplay(d, e, !1)
			}).on("pageAndSize" + j, function (a, c, f) {
				a.stopPropagation(), h.page = (parseInt(c, 10) || 1) - 1, b.setPageSize(d, parseInt(f, 10) || h.setSize || 10, e), b.moveToPage(d, h, !0), b.hideRows(d, e), b.updatePageDisplay(d, e, !1)
			}), f = [k.first, k.prev, k.next, k.last], g = ["moveToFirstPage", "moveToPrevPage", "moveToNextPage", "moveToLastPage"], e.debug && !h.$container.length && console.warn("Pager: >> Container not found"), h.$container.find(f.join(",")).attr("tabindex", 0).off("click" + j).on("click" + j, function (c) {
				c.stopPropagation();
				var e, j = a(this), k = f.length;
				if (!j.hasClass(i.pager_css.disabled))for (e = 0; k > e; e++)if (j.is(f[e])) {
					b[g[e]](d, h);
					break
				}
			}), h.$goto.length ? h.$goto.off("change" + j).on("change" + j, function () {
				h.page = a(this).val() - 1, b.moveToPage(d, h, !0), b.updatePageDisplay(d, e, !1)
			}) : e.debug && console.warn("Pager: >> Goto selector not found"), h.$size.length ? (h.$size.find("option").removeAttr("selected"), h.$size.off("change" + j).on("change" + j, function () {
				return h.$size.val(a(this).val()), a(this).hasClass(i.pager_css.disabled) || (b.setPageSize(d, parseInt(a(this).val(), 10), e), b.changeHeight(d, e)), !1
			})) : e.debug && console.warn("Pager: >> Size selector not found")
			...
			if (i && !g.pager_ajaxUrl)if (a.isEmptyObject(c.cache))h.filteredRows = h.totalRows = c.$tbodies.eq(0).children("tr").not(g.pager_countChildRows ? "" : "." + c.cssChildRow).length;
			else for (h.filteredRows = 0, d = c.cache[0].normalized, f = d.length, e = 0; f > e; e++)h.filteredRows += h.regexRows.test(d[e][c.columns].$row[0].className) ? 0 : 1;
			else i || (h.filteredRows = h.totalRows)
			...
			if (!e.pager.initializing) {
				...
				b.pagerArrows(e), b.fixHeight(d, e), n.initialized && f !== !1 && (e.debug && console.log("Pager: Triggering pagerComplete"), e.$table.trigger("pagerComplete", e), m.pager_savePages && c.storage && c.storage(d, m.pager_storageKey, {
					page: n.page,
					size: n.size
				}))
			}
			...
			if (q.push(k), m) {
				...
				q = a.grep(q, function (b, c) {
					return a.inArray(b, q) === c
				}), h = q.length, i = f.length, h - i > l / 2 && h + i > j.pager_maxOptionSize && (g = Math.floor(h / 2) - Math.floor(i / 2), Array.prototype.splice.apply(q, [g, i])), q = q.concat(f)
			}
			return q = a.grep(q, function (b, c) {
				return a.inArray(b, q) === c
			}).sort(function (a, b) {
				return a - b
			})
			...
			e.size = parseInt(e.$size.val(), 10) || e.size || e.setSize || 10, a.data(c, "pagerLastSize", e.size), b.pagerArrows(d), d.widgetOptions.pager_removeRows || (b.hideRows(c, d), d.$table.on("sortEnd filterEnd ".split(" ").join(f + " "), function () {
				b.hideRows(c, d)
			}))
			...
			if (a.isFunction(k.pager_ajaxProcessing)) {
				...
				if (c.showError(e), i)f.debug && console.error("Pager: >> Ajax Error", g, h, i), c.showError(e, g, h, i), f.$tbodies.eq(0).children("tr").detach(), j.totalRows = 0;
				else {
					if (a.isArray(B) ? (n = isNaN(B[0]) && !isNaN(B[1]), x = B[n ? 1 : 0], j.totalRows = isNaN(x) ? j.totalRows || 0 : x, f.totalRows = f.filteredRows = j.filteredRows = j.totalRows, v = 0 === j.totalRows ? [] : B[n ? 0 : 1] || [], u = B[2]) : (j.ajaxData = B, f.totalRows = j.totalRows = B.total, f.filteredRows = j.filteredRows = "undefined" != typeof B.filteredRows ? B.filteredRows : B.total, u = B.headers, v = B.rows || []), w = v && v.length, v instanceof jQuery)k.pager_processAjaxOnInit && (f.$tbodies.eq(0).empty(), f.$tbodies.eq(0).append(v));
					else if (w) {
						for (l = 0; w > l; l++) {
				...
				f.showProcessing && c.isProcessing(e), j.totalPages = Math.ceil(j.totalRows / (j.size || j.setSize || 10)), j.last.totalRows = j.totalRows, j.last.currentFilters = j.currentFilters, j.last.sortList = (f.sortList || []).join(","), j.initializing = !1, b.updatePageDisplay(e, f, !1), c.updateCache(f, function () {
					j.initialized && setTimeout(function () {
						f.debug && console.log("Pager: Triggering pagerChange"), z.trigger("pagerChange", j), c.applyWidget(e), b.updatePageDisplay(e, f)
					}, 0)
				})
			}
			j.initialized || c.applyWidget(e)
		}, getAjax: function (d, e) {
			var f, g = b.getAjaxUrl(d, e), h = a(document), i = e.namespace + "pager", j = e.pager;
			"" !== g && (e.showProcessing && c.isProcessing(d, !0), h.on("ajaxError" + i, function (a, c, f, g) {
				b.renderAjax(null, d, e, c, f, g), h.off("ajaxError" + i)
			}), f = ++j.ajaxCounter, j.last.ajaxUrl = g, j.ajaxObject.url = g, j.ajaxObject.success = function (a, c, g) {
				f < j.ajaxCounter || (b.renderAjax(a, d, e, g), h.off("ajaxError" + i), "function" == typeof j.oldAjaxSuccess && j.oldAjaxSuccess(a))
			}, e.debug && console.log("Pager: Ajax initialized", j.ajaxObject), a.ajax(j.ajaxObject))
		}, getAjaxUrl: function (b, c) {
			var d, e, f = c.pager, g = c.widgetOptions, h = g.pager_ajaxUrl ? g.pager_ajaxUrl.replace(/\{page([\-+]\d+)?\}/, function (a, b) {
				return f.page + (b ? parseInt(b, 10) : 0)
			}).replace(/\{size\}/g, f.size) : "", i = c.sortList, j = f.currentFilters || a(b).data("lastSearch") || [], k = h.match(/\{\s*sort(?:List)?\s*:\s*(\w*)\s*\}/), l = h.match(/\{\s*filter(?:List)?\s*:\s*(\w*)\s*\}/), m = [];
			if (k) {
			...
			if (j.cacheIndex = [], j.isDisabled = !1, j.initialized && (i.debug && console.log("Pager: Triggering pagerChange"), i.$table.trigger("pagerChange", i)), k.pager_removeRows || j.showAll) {
		...
		}, showAllRows: function (d, e) {
			var f, g, h, i = e.pager, j = e.widgetOptions;
			for (i.ajax ? b.pagerArrows(e, !0) : (a.data(d, "pagerLastPage", i.page), a.data(d, "pagerLastSize", i.size), i.page = 0, i.size = i.totalRows, i.totalPages = 1, i.showAll = !0, e.$table.addClass("pagerDisabled").removeAttr("aria-describedby").find("tr.pagerSavedHeightSpacer").remove(), b.renderTable(d, e.rowsCopy), i.isDisabled = !0, c.applyWidget(d), e.debug && console.log("Pager: Disabled")), g = i.$size.add(i.$goto).add(i.$container.find(".ts-startRow, .ts-page ")), h = g.length, f = 0; h > f; f++)g.eq(f).attr("aria-disabled", "true").addClass(j.pager_css.disabled)[0].disabled = !0
		}, updateCache: function (d) {
			var e = d.config, f = e.pager;
			c.updateCache(e, function () {
				if (!a.isEmptyObject(d.config.cache)) {
					var c, g = [], h = d.config.cache[0].normalized;
					for (f.totalRows = h.length, c = 0; c < f.totalRows; c++)g.push(h[c][e.columns].$row);
					e.rowsCopy = g, b.moveToPage(d, f, !0), f.last.currentFilters = [" "]
				}
			})
			...
			if (!e.isDisabled) {
				...
				e.ajax && !i.filter_initialized && c.hasWidget(d, "filter") || (b.calcFilters(d, h), g = Math.min(e.totalPages, e.filteredPages), e.page < 0 && (e.page = 0), e.page > g - 1 && 0 !== g && (e.page = g - 1), j.currentFilters = "" === (j.currentFilters || []).join("") ? [] : j.currentFilters, e.currentFilters = "" === (e.currentFilters || []).join("") ? [] : e.currentFilters, (j.page !== e.page || j.size !== e.size || j.totalRows !== e.totalRows || (j.currentFilters || []).join(",") !== (e.currentFilters || []).join(",") || (j.ajaxUrl || "") !== (e.ajaxObject.url || "") || (j.optAjaxUrl || "") !== (i.pager_ajaxUrl || "") || j.sortList !== (h.sortList || []).join(",")) && (h.debug && console.log("Pager: Changing to page " + e.page), e.last = {
					page: e.page,
					size: e.size,
					sortList: (h.sortList || []).join(","),
					totalRows: e.totalRows,
					currentFilters: e.currentFilters || [],
					ajaxUrl: e.ajaxObject.url || "",
					optAjaxUrl: i.pager_ajaxUrl
				}, e.ajax ? b.getAjax(d, h) : e.ajax || b.renderTable(d, h.rowsCopy), a.data(d, "pagerLastPage", e.page), e.initialized && f !== !1 && (h.debug && console.log("Pager: Triggering pageMoved"), h.$table.trigger("pageMoved", h), c.applyWidget(d), !e.ajax && d.isUpdating && (h.debug && console.log("Pager: Triggering updateComplete"), h.$table.trigger("updateComplete", [d, !0])))))
			}
		...
		}, moveToFirstPage: function (a, c) {
			c.page = 0, b.moveToPage(a, c, !0)
		}, moveToLastPage: function (a, c) {
			c.page = Math.min(c.totalPages, c.filteredPages) - 1, b.moveToPage(a, c, !0)
		}, moveToNextPage: function (a, c) {
			c.page++, c.page >= Math.min(c.totalPages, c.filteredPages) - 1 && (c.page = Math.min(c.totalPages, c.filteredPages) - 1), b.moveToPage(a, c, !0)
		}, moveToPrevPage: function (a, c) {
			c.page--, c.page <= 0 && (c.page = 0), b.moveToPage(a, c, !0)
		}, destroyPager: function (a, d, e) {
